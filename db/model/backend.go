...
func (b *Backend) Put(key string, value interface{}, lock ...bool) error {
	b.Lock()
	defer b.Unlock()
	//log.Info("================inside backend Put==============",*b)

	formattedPath := b.makePath(key)
//	log.Debugw("putting-key", log.Fields{"key": key, "value": string(value.([]byte)), "path": formattedPath, "lock": lock})
	//log.Info("================inside backend Put==============formattedPath,value",formattedPath,value)
...
func (b *Backend) CreateWatchForSubKeys(key string) chan *kvstore.Event {
	b.Lock()
	defer b.Unlock()
	...
	return b.Client.WatchForSubKeys(formattedPath)
}
